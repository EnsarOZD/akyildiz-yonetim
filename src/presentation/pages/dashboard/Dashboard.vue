<template>
    ...
    <div class="max-w-7xl mx-auto space-y-5">
        ...
        <template #actions>
          <div class="flex items-center gap-2">
            <select
              ...
              class="select select-sm bg-white border-slate-200 text-xs font-bold text-slate-600 rounded-lg h-9 min-h-0"
            >
            ...
            <button
              ...
              :disabled="loading"
              class="btn btn-sm bg-white border-slate-200 text-slate-600 hover:bg-slate-50 rounded-lg h-9 px-3 gap-2 normal-case font-bold text-xs shadow-sm disabled:opacity-50"
            >
      ...
      <template v-if="loading">
        <div class="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-5">
          ...
          <div class="app-card p-5 animate-pulse border-l-4 border-l-red-200">
            <div class="space-y-3">
              <div class="h-3 bg-slate-100 rounded w-2/5"></div>
              <div class="h-8 bg-slate-100 rounded w-3/5"></div>
            </div>
          </div>
        </div>
      </template>
      ...
      <!-- ─── Error state ────────────────────────────── -->
      <div v-else-if="error" class="app-card p-5 border-red-200 flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
          <p class="text-sm font-semibold text-slate-800">Veri yüklenemedi</p>
          <p class="text-xs text-slate-500 mt-0.5">{{ error }}</p>
          <button @click="loadDashboardData" class="mt-3 text-xs font-semibold text-brand-600 hover:underline">Tekrar dene</button>
        </div>
      </div>
      ...
      <template v-else>
        ...
        <div v-if="notificationsStore.latestAnnouncement"
          class="app-card flex flex-col sm:flex-row items-stretch overflow-hidden border-violet-200">
          ...
          <div class="flex-1 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div class="min-w-0">
              <p class="text-xs font-semibold text-violet-600 uppercase tracking-wider flex items-center gap-1.5 mb-1">
                <span class="w-1.5 h-1.5 rounded-full bg-violet-500 animate-pulse"></span>
                Son Duyuru
              </p>
              <p class="text-sm font-semibold text-slate-800 line-clamp-1">{{ notificationsStore.latestAnnouncement.title }}</p>
              <p class="text-xs text-slate-500 line-clamp-1 mt-0.5">{{ notificationsStore.latestAnnouncement.message }}</p>
            </div>
            <router-link to="/notifications"
              class="flex-shrink-0 inline-flex items-center gap-1 text-xs font-semibold text-violet-600 hover:underline">
              Görüntüle
        ...
        <template v-if="userRole === 'admin' || userRole === 'manager'">
          ...
          <section class="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-5">
            ...
            <div class="app-card p-6">
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-8 sm:gap-6">
                ...
                <router-link to="/transactions" class="flex flex-col items-start gap-3 group">
                  ...
                  <div>
                    <p class="text-[11px] font-black text-[#8298ab] uppercase tracking-widest mb-1 shadow-sm">Kasa Bakiyesi</p>
                    <p class="text-xl sm:text-[28px] font-black text-slate-800 leading-tight tabular-nums tracking-tighter">{{ formatCurrency(balance) }}</p>
                  </div>
                </router-link>

                <!-- Toplam Gelir -->
                <router-link to="/payments" class="flex flex-col items-start gap-3 group border-t sm:border-t-0 sm:border-l border-slate-100 pt-6 sm:pt-0 sm:pl-6">
                  <div class="w-10 h-10 rounded-xl bg-brand-500/10 text-brand-600 flex items-center justify-center group-hover:scale-110 transition-all duration-300">
                  ...
                  <div>
                    <p class="text-[11px] font-black text-[#8298ab] uppercase tracking-widest mb-1 shadow-sm">Toplam Gelir</p>
                    <p class="text-xl sm:text-[28px] font-black text-brand-600 leading-tight tabular-nums tracking-tighter">{{ formatCurrency(totalIncome) }}</p>
                    <p class="text-[11px] font-bold text-slate-500 mt-1">+{{ formatCurrency(thisMonthTenantPayments) }} <span class="font-medium opacity-80 uppercase tracking-tighter">bu ay</span></p>
                  </div>
                </router-link>

                <!-- Toplam Gider -->
                <router-link to="/expenses" class="flex flex-col items-start gap-3 group border-t sm:border-t-0 sm:border-l border-slate-100 pt-6 sm:pt-0 sm:pl-6">
                  ...
                  <div>
                    <p class="text-[11px] font-black text-[#8298ab] uppercase tracking-widest mb-1 shadow-sm">Toplam Gider</p>
                    ...
                    <p class="text-[11px] font-bold text-slate-500 mt-1 uppercase tracking-tighter">{{ totalExpensesCount }} kalem</p>
                  </div>
                </router-link>
            ...
            <router-link to="/overdue"
              ...
              <div>
                <p class="text-xs font-black text-slate-500 mb-1 uppercase tracking-widest">Geciken Toplam</p>
                <p class="text-[36px] font-black text-slate-800 tabular-nums tracking-tighter leading-none mb-2">
                  {{ formatCurrency(overdueTotalAmount) }}
                </p>
                <div v-if="overdueCount > 0 && oldestOverdueDate" class="flex items-center gap-1.5 mt-2 text-xs text-slate-500">
                  <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                  En eski: <span class="font-bold text-red-500">{{ formatDate(oldestOverdueDate) }}</span>
                </div>
              </div>

              <div class="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between group">
          ...
          <section class="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-5">
              ...
              <div class="p-6 overflow-x-auto">
                <div class="h-40 flex items-end gap-3 min-w-[380px]">
                  <div v-for="(month, index) in monthlyData" :key="index" class="flex-1 flex flex-col items-center gap-2">
                    ...
                    <span class="text-[10px] font-black text-[#8298ab] whitespace-nowrap tracking-wider">{{ month.month }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <router-link to="/utilities" class="app-card p-6 hover:shadow-xl transition-all duration-300 flex flex-col justify-between">
              <div>
                <h4 class="text-[14px] font-bold text-slate-700 mb-5 flex items-center gap-2">
                  <span class="w-1 h-3.5 bg-brand-500 rounded-full"></span>
                  Bekleyen Alacaklar
                </h4>
                <div class="space-y-4">
                  <div class="flex items-center justify-between pb-3 border-b border-slate-50">
                    <span class="flex items-center gap-2.5 text-[13px] font-medium text-slate-600">
                      <span class="w-2 h-2 rounded-full bg-emerald-500"></span>Aidat
                    </span>
                    <span class="text-[14px] font-bold text-slate-800 tabular-nums">{{ formatCurrency(totalAidatDebt) }}</span>
                  </div>
                  <div class="flex items-center justify-between pb-3 border-b border-slate-50">
                    <span class="flex items-center gap-2.5 text-[13px] font-medium text-slate-600">
                      <span class="w-2 h-2 rounded-full bg-amber-500"></span>Elektrik
                    </span>
                    <span class="text-[14px] font-bold text-slate-800 tabular-nums">{{ formatCurrency(totalElectricityDebt) }}</span>
                  </div>
                  <div class="flex items-center justify-between pb-3 border-b border-slate-50">
                    <span class="flex items-center gap-2.5 text-[13px] font-medium text-slate-600">
                      <span class="w-2 h-2 rounded-full bg-brand-500"></span>Su
                    </span>
                    <span class="text-[14px] font-bold text-slate-800 tabular-nums">{{ formatCurrency(totalWaterDebt) }}</span>
                  </div>
                </div>
              </div>
              <div class="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
                <span class="text-xs font-bold text-slate-500 uppercase tracking-widest">Toplam</span>
          ...
          <section>
            <h2 class="text-[11px] font-bold text-[#8298ab] uppercase tracking-[1.5px] mb-4 px-0.5">Hızlı İşlemler</h2>
            <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-7 gap-4">
              ...
              <div class="dropdown dropdown-end sm:dropdown-bottom group">
                <div tabindex="0" role="button"
                  class="app-card p-5 flex flex-col items-center gap-3 cursor-pointer transition-all hover:-translate-y-1 hover:bg-slate-50">
                  ...
                  <span class="text-xs font-bold text-slate-700 text-center leading-tight">Borç Ekle</span>
                </div>
                <ul tabindex="0" class="dropdown-content z-[2] menu p-2 shadow-2xl bg-white rounded-xl w-48 mt-2 border border-slate-200 text-sm">
                  <li><a @click="openManualDebtModal(0)" class=" hover:bg-emerald-500/10">Aidat Ekle</a></li>
                  <li><a @click="openManualDebtModal(1)" class=" hover:bg-amber-500/10">Elektrik Ekle</a></li>
                  <li><a @click="openManualDebtModal(2)" class=" hover:bg-brand-500/10">Su Ekle</a></li>
                </ul>
              </div>

              <!-- Tahsilat Al -->
              <router-link to="/payments" class="app-card p-5 flex flex-col items-center gap-3 group transition-all hover:-translate-y-1 hover:bg-slate-50">
                ...
                <span class="text-xs font-bold text-slate-700 text-center leading-tight">Tahsilat Al</span>
              </router-link>

              <!-- Gider Ekle -->
              <router-link to="/expenses" class="app-card p-5 flex flex-col items-center gap-3 group transition-all hover:-translate-y-1 hover:bg-slate-50">
                ...
                <span class="text-xs font-bold text-slate-700 text-center leading-tight">Gider Ekle</span>
              </router-link>

              <!-- Kiracılar -->
              <router-link to="/tenants" class="app-card p-5 flex flex-col items-center gap-3 group transition-all hover:-translate-y-1 hover:bg-slate-50">
                ...
                <span class="text-xs font-bold text-slate-700 text-center leading-tight">Kiracılar</span>
              </router-link>

              <!-- Raporlar -->
              <router-link to="/reports" class="app-card p-5 flex flex-col items-center gap-3 group transition-all hover:-translate-y-1 hover:bg-slate-50">
                ...
                <span class="text-xs font-bold text-slate-700 text-center leading-tight">Raporlar</span>
              </router-link>

              <!-- Talepler -->
              <router-link to="/service-requests" class="app-card p-5 flex flex-col items-center gap-3 group transition-all hover:-translate-y-1 hover:bg-slate-50">
                <div class="w-11 h-11 rounded-xl bg-brand-500/10 text-brand-600 flex items-center justify-center group-hover:scale-110 transition-transform">
                ...
                <span class="text-xs font-bold text-slate-700 text-center leading-tight">Talepler</span>
              </router-link>

              <!-- Duyuru Yap -->
              <button @click="showAnnouncementModal = true"
                class="app-card p-5 flex flex-col items-center gap-3 group transition-all hover:-translate-y-1 hover:bg-slate-50 w-full">
                <div class="w-11 h-11 rounded-xl bg-violet-500/10 text-violet-500 flex items-center justify-center group-hover:scale-110 transition-transform">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z"/></svg>
                </div>
                <span class="text-xs font-bold text-slate-700 text-center leading-tight">Duyuru Yap</span>
              </button>
        ...
        <section v-else-if="userRole?.toLowerCase() === 'observer'" class="grid grid-cols-1 sm:grid-cols-3 gap-5">
          <div class="app-card p-5 border-l-4 border-amber-400">
            <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Bekleyen Borç</p>
            <p class="text-2xl font-bold text-slate-800">{{ formatCurrency(totalUtilityDebts) }}</p>
          ...
          <div class="app-card p-5" :class="overdueCount > 0 ? 'border-l-4 border-red-400' : ''">
            <p class="text-xs font-semibold uppercase tracking-wider mb-1" :class="overdueCount > 0 ? 'text-red-500' : 'text-slate-500'">Geciken Borçlar</p>
            <p class="text-2xl font-bold text-slate-800">{{ formatCurrency(overdueTotalAmount) }}</p>
          ...
          <div class="app-card p-5 border-l-4 border-emerald-400">
            <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Toplam Tahsilat</p>
            <p class="text-2xl font-bold text-slate-800">{{ formatCurrency(totalIncome) }}</p>
            <p class="text-xs text-slate-500 mt-1">+{{ formatCurrency(thisMonthTenantPayments) }} bu ay</p>
          </div>
        </section>
        ...
        <section class="app-card !p-0 overflow-hidden">
          <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            ...
            <span class="text-[11px] font-bold text-[#8298ab] uppercase tracking-wider">{{ overdueCount }} ADET</span>
          </div>
          ...
          <div v-if="overdueItems.length > 0">
            <div v-for="item in overdueItems" :key="item.id"
              ...
              <div class="min-w-0 flex-1">
                <p class="text-[12.5px] font-bold text-slate-800 truncate uppercase tracking-wide">{{ item.company || item.displayName }}</p>
                <p class="text-[11px] text-slate-500 mt-0.5 opacity-80">— {{ item.typeLabel }}</p>
              </div>
              <div class="text-right flex-shrink-0">
                <p class="text-[14px] font-extrabold text-red-500 tabular-nums">{{ formatCurrency(item.amount) }}</p>
                <p class="text-[10px] font-bold text-[#8298ab] mt-0.5">{{ formatDate(item.dueDate) }}</p>
              </div>
            </div>
          </div>
          ...
          <div v-else class="px-5 py-12 flex flex-col items-center gap-3">
            ...
            <p class="text-[13px] font-semibold text-slate-700">Geciken ödeme bulunmuyor</p>
          </div>

          <div v-if="overdueCount > 0" class="px-6 py-3 border-t border-slate-100">
            <router-link to="/overdue"
              class="inline-flex items-center gap-1.5 text-[12px] font-bold text-brand-600 hover:opacity-80 transition-opacity">
              Tümünü gör
        ...
        <section class="app-card !p-0 overflow-hidden">
          <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h2 class="text-[14px] font-bold text-slate-800 flex items-center gap-2">
              <svg class="w-4 h-4 text-brand-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
              Son İşlemler
            </h2>
            <select v-model="dashboardType"
              class="select select-xs bg-slate-50 border-slate-200 text-[11px] font-bold text-slate-500 rounded-lg h-7 min-h-0">
          ...
          <div v-if="recentActivities.length > 0">
            <div v-for="activity in recentActivities" :key="activity.id"
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="text-[11px] text-slate-500 mt-0.5 opacity-80 uppercase tracking-wide">
          ...
          <div v-else class="px-5 py-12 flex flex-col items-center gap-3">
            <div class="w-12 h-12 rounded-2xl bg-slate-100 text-[#8298ab] flex items-center justify-center">
            ...
            <p class="text-[13px] font-semibold text-slate-500">Henüz işlem bulunmuyor</p>
          </div>
  ...
  <div v-else class="flex items-center justify-center min-h-screen">
    <div class="text-center">
      ...
      <p class="text-sm font-medium text-slate-700">Bu sayfayı görüntüleme yetkiniz bulunmamaktadır.</p>
      <p class="text-xs text-[#8298ab] mt-1">Mevcut rol: {{ userRole || 'Tanımlanmamış' }}</p>
    </div>
  </div>
</template>
