  ...
  <div class="app-card !p-0 overflow-hidden">
    ...
    <div class="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="text-[15px] font-black text-slate-800 leading-none uppercase tracking-tight">Genel Borç Durumu</h2>
          <p class="text-[10px] text-[#8298ab] mt-1.5 font-bold uppercase tracking-[0.15em]">{{ filteredDebts.length }} Kayıt Listeleniyor</p>
        </div>
      </div>
      
      <div class="relative max-w-xs w-full">
        <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center text-[#8298ab]">
        ...
        <input 
          ...
          class="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 text-[13px] font-bold text-slate-700 rounded-xl focus:ring-2 focus:ring-brand-500/20 focus:border-brand-600 outline-none transition-all placeholder:text-[#8298ab]"
        >
      </div>
    </div>
      ...
      <table class="w-full border-collapse">
        <thead class="bg-slate-50/50 border-b border-slate-100">
          <tr class="text-slate-500">
            ...
            <th scope="col" @click="sortBy('aidatDebt')" class="text-right px-4 py-4 text-[10px] font-black uppercase tracking-[1.5px] cursor-pointer hover:text-brand-600 transition-colors">
              AİDAT <span v-if="sortKey === 'aidatDebt'" class="ml-1">{{ sortOrder === 1 ? '↑' : '↓' }}</span>
            </th>
            <th scope="col" @click="sortBy('electricityDebt')" class="text-right px-4 py-4 text-[10px] font-black uppercase tracking-[1.5px] cursor-pointer hover:text-brand-600 transition-colors">
              ELEKTRİK <span v-if="sortKey === 'electricityDebt'" class="ml-1">{{ sortOrder === 1 ? '↑' : '↓' }}</span>
            </th>
            <th scope="col" @click="sortBy('waterDebt')" class="text-right px-4 py-4 text-[10px] font-black uppercase tracking-[1.5px] cursor-pointer hover:text-brand-600 transition-colors">
              SU <span v-if="sortKey === 'waterDebt'" class="ml-1">{{ sortOrder === 1 ? '↑' : '↓' }}</span>
            </th>
            <th scope="col" @click="sortBy('totalDebt')" class="text-right px-6 py-4 text-[10px] font-black uppercase tracking-[1.5px] cursor-pointer hover:text-amber-500 transition-colors text-amber-600">
        ...
        <tbody class="divide-y divide-[#ecf3f9]">
          <tr v-if="loading" v-for="i in 3" :key="i" class="animate-pulse">
            <td colspan="7" class="py-5 px-6"><div class="h-4 bg-slate-100 rounded w-full"></div></td>
          </tr>
          ...
          >
            <td class="px-6 py-4">
              <div class="text-[13px] font-black text-slate-800 uppercase tracking-wide group-hover:text-brand-600 transition-colors">{{ item.displayName }}</div>
            </td>
            ...
            <td class="px-4 py-4 text-center">
              <span class="text-[11px] font-black text-slate-500 bg-slate-100 px-2 py-0.5 rounded shadow-sm tabular-nums">{{ item.apartmentNumber || '-' }}</span>
            </td>
            <td class="px-4 py-4 text-right">
              <span class="text-[13.5px] font-bold tabular-nums" 
                :class="item.aidatDebt > 0 ? 'text-slate-800' : 'text-[#8298ab]'">
                {{ formatCurrency(item.aidatDebt) }}
              </span>
            </td>
            <td class="px-4 py-4 text-right">
              <span class="text-[13.5px] font-bold tabular-nums" 
                :class="item.electricityDebt > 0 ? 'text-slate-800' : 'text-[#8298ab]'">
                {{ formatCurrency(item.electricityDebt) }}
              </span>
            </td>
            <td class="px-4 py-4 text-right">
              <span class="text-[13.5px] font-bold tabular-nums" 
                :class="item.waterDebt > 0 ? 'text-slate-800' : 'text-[#8298ab]'">
    ...
    <!-- Info Footer -->
    <div class="px-6 py-4 bg-slate-50/50 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-3">
      <div class="text-[11px] font-bold text-[#8298ab] uppercase tracking-widest">
        Aktif Filtreleme: <span class="text-slate-600">{{ search || 'Tümü' }}</span>
      </div>
