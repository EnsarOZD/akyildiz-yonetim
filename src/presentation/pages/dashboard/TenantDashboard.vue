      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <section class="lg:col-span-12 xl:col-span-7 space-y-4 animate-slide-up">
          <div class="flex items-center justify-between px-1">
            ...
            <span class="text-[10px] font-black text-brand-600 bg-brand-500/10 px-2.5 py-1 rounded-full uppercase">{{ pendingDebts.length }} Kalem</span>
          </div>
          ...
          <div v-if="pendingDebts.length > 0" class="space-y-3">
            <div
              ...
              :key="debt.id"
              class="app-card hover:border-brand-600/30 group p-4 flex items-center justify-between gap-4"
            >
              <div class="flex items-center gap-4 min-w-0">
                ...
                <div class="min-w-0">
                  <p class="text-sm font-black text-slate-800 uppercase tracking-tight">{{ getDebtTypeLabel(debt.type) }}</p>
                  <p class="text-[11px] font-bold text-[#8298ab]">Vade: {{ formatDate(debt.dueDate) }}</p>
                </div>
              </div>
              <div class="text-right shrink-0">
                <p class="text-base font-black text-brand-600 tabular-nums">{{ formatCurrency(debt.remainingAmount ?? debt.amount ?? 0) }}</p>
                <div class="flex items-center justify-end gap-1.5 mt-0.5">
                  <span class="text-[10px] font-bold text-[#8298ab] uppercase tracking-widest">{{ debt.periodMonth }}/{{ debt.periodYear }}</span>
          ...
          <div v-else class="app-card border-dashed py-12 flex flex-col items-center text-center">
            ...
            <h3 class="text-sm font-black text-slate-800 uppercase tracking-tight">Borcunuz Bulunmuyor</h3>
            <p class="text-[11px] font-bold text-[#8298ab] mt-1">Her şey yolunda! Tüm ödemeleriniz güncel.</p>
          </div>
        </section>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="app-card p-5 space-y-3">
              ...
              <div>
                ...
                <p class="text-xl font-black text-slate-800 tabular-nums">{{ formatCurrency(overdueDebt) }}</p>
              </div>
            </div>
            <div class="app-card p-5 space-y-3">
              <div class="p-2.5 w-10 h-10 rounded-xl bg-brand-500/10 text-brand-600 flex items-center justify-center">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
              </div>
              <div>
                <p class="text-premium-label mb-1">Son Ödeme</p>
                <p class="text-xl font-black text-slate-800 tabular-nums">{{ lastPaymentAmount > 0 ? formatCurrency(lastPaymentAmount) : '-' }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              <router-link to="/service-requests" class="app-card p-5 group hover:bg-brand-500 transition-colors duration-300">
                <div class="w-10 h-10 rounded-xl bg-brand-500/10 text-brand-600 group-hover:bg-white/20 group-hover:text-white flex items-center justify-center mb-3">
                ...
                <p class="text-xs font-black text-slate-800 uppercase group-hover:text-white tracking-widest">ARIZA BİLDİR</p>
              </router-link>
              <router-link to="/profile" class="app-card p-5 group hover:bg-indigo-500 transition-colors duration-300">
                ...
                <p class="text-xs font-black text-slate-800 uppercase group-hover:text-white tracking-widest">PROFİLİM</p>
              </router-link>
            </div>
          </div>

          <div v-if="tenantInfo?.flats?.length > 0" class="app-card bg-slate-50 border-none p-5">
            <div class="flex items-center gap-4">
              <div class="w-10 h-10 rounded-full bg-white flex items-center justify-center text-brand-600 shadow-sm shrink-0">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
              </div>
              <div class="min-w-0 flex-1">
                <p class="text-[10px] font-black text-[#8298ab] uppercase tracking-[2px]">KAYITLI ÜNİTELER</p>
                <div class="flex flex-wrap gap-x-3 mt-1">
                  <span v-for="flat in tenantInfo.flats" :key="flat.id" class="text-sm font-black text-brand-600">
                    #{{ flat.code }}
                  </span>
                </div>
              </div>
