  ...
  <div class="p-4 sm:p-6 min-h-screen pb-24 md:pb-6">
    ...
    <div class="app-card !p-0">
      ...
      <div v-if="notificationsStore.loading && notificationsStore.items.length === 0" class="py-16 flex flex-col items-center gap-3">
        <span class="loading loading-spinner loading-md text-brand-600"></span>
        <p class="text-sm text-[#8298ab]">Bildirimler yükleniyor…</p>
      </div>
      ...
      <div v-else-if="filteredItems.length === 0" class="py-16 text-center px-4">
        <div class="w-14 h-14 rounded-2xl bg-slate-100 flex items-center justify-center mx-auto mb-3">
          <svg class="w-7 h-7 text-[#8298ab]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <p class="text-xs text-[#8298ab] mt-1">
      ...
      <div v-else class="divide-y divide-[#ecf3f9]">
        <div
          ...
          :class="!item.isRead ? 'bg-brand-50/50' : ''"
        >
          <!-- Okunmamış nokta -->
          <span
            v-if="!item.isRead"
            class="absolute left-1.5 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-brand-500 shrink-0"
          />
          ...
          <div class="shrink-0 w-9 h-9 rounded-xl flex items-center justify-center mt-0.5" :class="getTypeBg(item.type)">
            ...
            <svg v-else-if="item.type === 'Announcement'" class="w-4 h-4" :class="getTypeColor(item.type)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 0 1 7 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 0 1 -1.564 -.317z"/>
            </svg>
            <!-- Default bell -->
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-start justify-between gap-2">
              <p class="text-sm font-semibold text-slate-800 leading-tight" :class="!item.isRead ? 'text-slate-900' : ''">
                {{ item.title }}
              </p>
              <span class="text-[10px] text-[#8298ab] shrink-0 mt-0.5">{{ formatDate(item.createdAt) }}</span>
            </div>
            <p class="text-xs text-slate-500 mt-0.5 leading-relaxed line-clamp-2">
              {{ item.message }}
            </p>
            <!-- Tıklanabilir hint -->
            <div v-if="item.type === 'Debt'" class="flex items-center gap-1 mt-1">
              <span class="text-[10px] text-brand-600 font-medium">Dashboard'a git</span>
      ...
      <!-- Daha Fazla Yükle -->
      <div v-if="hasMore" class="border-t border-slate-100">
        <button
          @click="notificationsStore.fetchMore"
          :disabled="notificationsStore.loading"
          class="w-full py-3 text-sm font-medium text-brand-600 hover:bg-slate-50 transition-colors"
        >
      ...
      <div class="space-y-6">
        <div class="flex flex-col items-center text-center space-y-4">
          ...
          <div class="space-y-2 px-2">
            <h5 class="text-sm font-black text-[#16283a] uppercase tracking-tight leading-relaxed">
              Kritik İşlem Onayı
            </h5>
            <p class="text-[11px] text-[#8298ab] font-medium leading-relaxed italic">
              Sistemdeki <span class="text-red-600 font-bold">tüm kullanıcılara ait</span> bildirim kayıtları kalıcı olarak silinecektir. Bu işlemin geri dönüşü yoktur.
            </p>
          </div>
        </div>
      </div>
...
const getTypeBg = (type) => {
    switch (type) {
        ...
        case 'Payment':
        case 'PaymentCreated': return 'bg-emerald-100'
...
const getTypeColor = (type) => {
    switch (type) {
        ...
        case 'Payment':
        case 'PaymentCreated': return 'text-emerald-600'
        case 'Announcement':   return 'text-amber-600'
