  ...
  <div class="p-4 sm:p-6 min-h-screen pb-24 md:pb-6">
    ...
    <div class="app-card !p-0">
      ...
      <div v-if="notificationsStore.loading && notificationsStore.items.length === 0" class="py-16 flex flex-col items-center gap-3">
        <span class="loading loading-spinner loading-md text-brand-500"></span>
        <p class="text-sm text-slate-400">Bildirimler yükleniyor…</p>
      </div>

      <!-- Boş -->
      <div v-else-if="filteredItems.length === 0" class="py-16 text-center px-4">
        <div class="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-[#1c2238] flex items-center justify-center mx-auto mb-3">
          <svg class="w-7 h-7 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
              d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0"/>
          </svg>
        </div>
        <p class="text-sm font-medium text-slate-500 dark:text-[#9aa0b4]">
      ...
      <div v-else class="divide-y divide-slate-100 dark:divide-white/[0.06]/50">
        <div
          ...
          :class="!item.isRead ? 'bg-brand-50/50 dark:bg-blue-900/10' : ''"
        >
          <!-- Okunmamış nokta -->
          <span
            v-if="!item.isRead"
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-start justify-between gap-2">
              <p class="text-sm font-semibold text-slate-800 dark:text-[#f1f3f9] leading-tight" :class="!item.isRead ? 'text-slate-900 dark:text-white' : ''">
                {{ item.title }}
              </p>
              <span class="text-[10px] text-slate-400 shrink-0 mt-0.5">{{ formatDate(item.createdAt) }}</span>
            </div>
            <p class="text-xs text-slate-500 dark:text-[#9aa0b4] mt-0.5 leading-relaxed line-clamp-2">
              {{ item.message }}
            </p>
            <!-- Tıklanabilir hint -->
            <div v-if="item.type === 'Debt'" class="flex items-center gap-1 mt-1">
              <span class="text-[10px] text-brand-500 font-medium">Dashboard'a git</span>
              <svg class="w-2.5 h-2.5 text-brand-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-if="hasMore" class="border-t border-slate-100 dark:border-slate-700/50">
        <button
          ...
          class="w-full py-3 text-sm font-medium text-brand-600 dark:text-brand-400 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors"
        >
...
const getTypeBg = (type) => {
    switch (type) {
        ...
        default:               return 'bg-brand-100 dark:bg-brand-500/[0.12]'
    }
}
