  ...
  >
    <div class="space-y-6">
      ...
      <div class="bg-brand-500/[0.05] border border-brand-600/20 rounded-2xl p-5">
        <div class="flex items-start gap-4">
          <div class="w-10 h-10 rounded-xl bg-brand-500/10 flex items-center justify-center shrink-0 text-brand-600 font-bold">ℹ️</div>
          <div>
            <h4 class="text-[11px] font-black uppercase tracking-widest text-brand-600 mb-1">Aktarım Talimatları</h4>
      ...
      <div v-if="!importResult" class="space-y-6">
        ...
        <div 
          ...
          :class="[
            isDragging 
              ? 'border-brand-600 bg-brand-500/[0.08] scale-[1.01]' 
        ...
        >
          ...
          <div v-if="!selectedFile" class="relative z-10">
            <div class="w-20 h-20 rounded-2xl bg-[#f6fafd] border border-[#d9e7f2] flex items-center justify-center mx-auto mb-6 shadow-xl transition-transform group-hover:scale-110 duration-500">
              <svg class="w-10 h-10 text-brand-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
              </svg>
            </div>
            <p class="text-[13px] font-black text-[#16283a] uppercase tracking-widest mb-2">Excel dosyanızı sürükleyin</p>
            <p class="text-xs text-[#8298ab] font-bold uppercase tracking-tighter mb-6">veya bilgisayarınızdan seçin</p>
          ...
          <div v-else class="relative z-10 flex flex-col items-center">
            ...
            <div class="mb-6">
              <p class="text-[14px] font-black text-[#16283a] uppercase tracking-tight mb-1">{{ selectedFile.name }}</p>
              <p class="text-[10px] font-bold text-[#8298ab] uppercase tracking-widest">{{ (selectedFile.size / 1024).toFixed(1) }} KB</p>
            </div>
            <button @click="selectedFile = null" class="btn btn-ghost !bg-red-500/10 text-red-600 hover:bg-red-500/20 px-6">
              Dosyayı Kaldır
            </button>
          </div>
        ...
        <div v-if="importError" class="bg-red-500/10 border border-red-500/20 p-5 rounded-2xl animate-shake">
          ...
          <p class="text-xs text-red-600 font-medium leading-relaxed italic">{{ importError }}</p>
        </div>
      </div>
      ...
      <div v-else class="space-y-6 animate-fade-in">
        <div class="grid grid-cols-2 gap-4">
          <div class="p-5 bg-green-500/5 border border-green-500/10 rounded-2xl text-center">
            <p class="text-[10px] font-black text-[#8298ab] uppercase tracking-widest mb-2">Başarılı İşlem</p>
            <p class="text-3xl font-black text-emerald-600 tabular-nums">{{ importResult.successCount }}</p>
          </div>
          <div class="p-5 bg-red-500/5 border border-red-500/10 rounded-2xl text-center">
            <p class="text-[10px] font-black text-[#8298ab] uppercase tracking-widest mb-2">Hatalı İşlem</p>
            <p class="text-3xl font-black text-red-600 tabular-nums">{{ importResult.failedCount }}</p>
          </div>
        </div>

        <!-- Hata Listesi -->
        <div v-if="importResult.errors && importResult.errors.length > 0" class="space-y-3">
          <h5 class="text-[10px] font-black text-[#8298ab] uppercase tracking-widest px-1">Hata Detayları</h5>
          <div class="max-h-56 overflow-y-auto border border-[#d9e7f2] bg-[#f6fafd] rounded-2xl p-4 custom-scrollbar">
            <ul class="space-y-3">
              <li v-for="(err, idx) in importResult.errors" :key="idx" class="flex items-start gap-3 p-3 bg-red-500/5 border border-red-500/10 rounded-xl">
                 <span class="text-red-600 font-black text-[10px] mt-1 shrink-0">#{{ idx + 1 }}</span>
        ...
        <div v-else class="flex flex-col items-center justify-center py-10 bg-green-500/5 border border-green-500/10 rounded-3xl animate-bounce-subtle">
          ...
          <p class="text-[13px] font-black text-emerald-600 uppercase tracking-widest px-6 text-center">
    ...
    <template #footer>
      <template v-if="!importResult">
        <button @click="$emit('close')" class="btn btn-ghost !bg-transparent border border-[#d9e7f2] text-[#5a7186] hover:bg-[#ecf3f9] flex-1">Vazgeç</button>
        <button 
          @click="startImport" 
