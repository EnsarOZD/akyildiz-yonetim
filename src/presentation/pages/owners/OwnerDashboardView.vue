    ...
    <div class="max-w-6xl mx-auto space-y-8">
      ...
      <section v-if="!loading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-slide-up">
        <div class="app-card border-none bg-slate-50 p-5 flex items-center gap-4">
          ...
          <div>
            <p class="text-premium-label mb-1">Kendi Borcum</p>
            <p class="text-xl font-black text-rose-500">{{ formatCurrency(data.totalOwnerDebt) }}</p>
          </div>
        </div>

        <div class="app-card border-none bg-slate-50 p-5 flex items-center gap-4">
          <div class="w-12 h-12 rounded-2xl bg-brand-500/10 text-brand-600 flex items-center justify-center shrink-0">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
          </div>
          <div>
            <p class="text-premium-label mb-1">Kiracı Bakiyesi</p>
            <p class="text-xl font-black text-slate-800">{{ formatCurrency(data.totalTenantDebt) }}</p>
          </div>
        </div>
      ...
      <section v-if="loading" class="grid grid-cols-1 md:grid-cols-4 gap-4 animate-pulse">
        <div v-for="i in 4" :key="i" class="app-card h-32 border-none bg-slate-100"></div>
      </section>
      ...
      <section v-else class="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-slide-up">
        ...
        <div class="lg:col-span-5 space-y-4">
          <div class="flex items-center justify-between px-1">
             ...
             <span class="text-[10px] font-black text-[#8298ab] uppercase tracking-widest">{{ data.myProperties.length }} Ünite</span>
          </div>
            ...
            <div v-for="prop in data.myProperties" :key="prop.flatId"
              ...
              :class="[
                ...
                selectedFlatId === prop.flatId 
                  ? 'bg-white border-brand-600/30 shadow-2xl shadow-brand-500/10 scale-[1.02]' 
              ...
              <div class="flex items-center justify-between relative z-10">
                <div class="flex items-center gap-4">
                  <div :class="[
                    ...
                    selectedFlatId === prop.flatId ? 'bg-brand-gradient text-white rotate-6' : 'bg-white text-[#8298ab]'
                  ]">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                  </div>
                  <div>
                    <h3 class="text-[15px] font-black tracking-tight" :class="selectedFlatId === prop.flatId ? 'text-brand-600' : 'text-slate-800'">
                       NO: {{ prop.code }}
                    </h3>
                    <p class="text-[10px] font-bold uppercase tracking-widest mt-1" :class="prop.isOccupied ? 'text-emerald-500' : 'text-[#8298ab]'">
                      {{ prop.isOccupied ? prop.tenantName : 'Boş / Müsait' }}
                    </p>
                  </div>
                </div>
                <div class="text-right">
                  <p class="text-sm font-black" :class="getPropertyDebtTotal(prop.flatId) > 0 ? 'text-rose-500' : 'text-[#8298ab]'">
                    {{ formatCurrency(getPropertyDebtTotal(prop.flatId)) }}
                  </p>
                  <p class="text-[9px] font-black uppercase text-[#8298ab] mt-1">TOPLAM BORÇ</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="lg:col-span-7">
          <transition name="fade-slide" mode="out-in">
            <div v-if="selectedFlat" :key="selectedFlat.flatId" class="app-card !p-0 overflow-hidden shadow-2xl shadow-slate-200 border-none h-full">
              <!-- Detail Hero Section -->
              <div class="p-8 bg-slate-50 border-b border-slate-100">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-6">
                  <div class="flex items-center gap-6">
                    ...
                    <div>
                      <h2 class="text-3xl font-black text-slate-800 tracking-tighter uppercase italic">Ünite {{ selectedFlat.code }}</h2>
                      <div class="flex items-center gap-3 mt-2">
                         <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest" :class="selectedFlat.isOccupied ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-200 text-[#8298ab]'">
                  ...
                  <div class="text-center sm:text-right">
                    <p class="text-[10px] font-black text-[#8298ab] uppercase tracking-[0.2em] mb-1">Cari Hesap Toplamı</p>
                    <p class="text-3xl font-black text-slate-800">{{ formatCurrency(getPropertyDebtTotal(selectedFlat.flatId)) }}</p>
                  </div>
                </div>
              </div>
              ...
              <div class="p-6">
                <div v-if="selectedPropertyDebts.length === 0" class="flex flex-col items-center justify-center py-20 text-center space-y-4">
                  ...
                  <p class="text-sm font-black text-[#8298ab] uppercase tracking-widest">Sıfır Bakiye / Borç Bulunmuyor</p>
                </div>
                ...
                <div v-else class="space-y-4">
                   ...
                   <div class="space-y-3">
                      <div v-for="debt in selectedPropertyDebts" :key="debt.id" 
                        class="p-5 flex items-center justify-between bg-slate-50 rounded-2xl group hover:scale-[1.01] transition-all">
                        <div class="flex items-center gap-5">
                          ...
                          <div>
                            <p class="text-sm font-black text-slate-800 uppercase tracking-tight">{{ debtTypeLabel(debt.type) }}</p>
                            <p class="text-[10px] font-black uppercase tracking-widest mt-1">
                              {{ debt.periodMonth }}/{{ debt.periodYear }}
                              <span class="mx-2 text-[#8298ab]">|</span>
                              <span :class="debt.isVacantDebt ? 'text-amber-500' : 'text-[#8298ab]'">{{ debt.isVacantDebt ? 'MÜLK SAHİBİ BORCU' : 'KİRACI BORCU' }}</span>
                            </p>
                          </div>
                        </div>
                        <div class="text-right">
                          <p class="text-[15px] font-black text-rose-500">- {{ formatCurrency(debt.remainingAmount) }}</p>
                          <p class="text-[9px] font-black text-[#8298ab] uppercase mt-1">{{ formatDate(debt.dueDate) }}</p>
                        </div>
                      </div>
                   </div>
                </div>
            ...
            <div v-else class="app-card h-[600px] flex flex-col items-center justify-center p-12 text-center border-dashed border-2 border-slate-200 bg-slate-50/30">
              <div class="w-24 h-24 rounded-full bg-slate-100 text-[#8298ab] flex items-center justify-center mb-8 relative">
                 <div class="absolute inset-0 rounded-full border-2 border-slate-200 animate-[ping_3s_linear_infinite]"></div>
              ...
              <h3 class="text-xl font-black text-slate-800 uppercase tracking-tighter">İzleme Merkezi</h3>
              <p class="text-xs font-bold text-[#8298ab] mt-3 max-w-[280px] uppercase tracking-widest leading-relaxed">Detaylı finansal verileri ve borç durumunu görmek için sol listeden bir ünite seçin.</p>
            </div>
          </transition>
        </div>
      </section>
      ...
      <section class="space-y-4 animate-slide-up">
        ...
        <div class="app-card !p-0 overflow-hidden shadow-xl shadow-slate-200 border-none">
          <div v-if="data.recentPayments.length === 0" class="p-12 text-center text-[#8298ab] font-bold uppercase text-[10px] tracking-widest">Henüz bir ödeme kaydı bulunmamaktadır.</div>
          <div v-else class="divide-y divide-[#ecf3f9]">
            <div v-for="p in data.recentPayments" :key="p.id" class="p-6 flex items-center justify-between group hover:bg-slate-50 transition-all">
              <div class="flex items-center gap-5">
                ...
                <div>
                  <p class="text-sm font-black text-slate-800 uppercase tracking-tight">{{ p.description || p.type || 'Sistem Tahsilatı' }}</p>
                  <p class="text-[10px] font-black text-[#8298ab] uppercase tracking-widest mt-1">{{ formatDate(p.paymentDate) }}</p>
                </div>
              </div>
              <div class="text-right">
