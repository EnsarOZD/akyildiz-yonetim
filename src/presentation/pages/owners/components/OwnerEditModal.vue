    ...
    <div class="modal-box max-w-lg bg-white dark:bg-[#0f1322] border border-gray-200 dark:border-white/[0.07]">
      ...
      <form @submit.prevent="submitForm" class="py-6 space-y-6">
        ...
        <div class="form-control">
          ...
          <input v-model="formData.name" type="text" placeholder="Mal sahibinin adı ve soyadı" class="input input-bordered w-full bg-white dark:bg-[#1c2238]" required />
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="form-control">
            ...
            <input v-model="formData.phone" v-mask="'+90 (###) ### ## ##'" type="tel" placeholder="+90 (5xx) xxx xx xx" class="input input-bordered w-full bg-white dark:bg-[#1c2238]" />
          </div>
          <div class="form-control">
            <label class="label">
              <span class="label-text font-semibold text-gray-700 dark:text-[#f1f3f9]">E-posta Adresi</span>
            </label>
            <input v-model="formData.email" type="email" placeholder="ornek@mail.com" class="input input-bordered w-full bg-white dark:bg-[#1c2238]" />
          </div>
        </div>
