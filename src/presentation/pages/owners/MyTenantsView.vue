    ...
    <div class="max-w-4xl mx-auto">
      ...
      <template v-else>
        ...
        <div v-if="occupiedFlats.length === 0" class="app-card p-12 text-center mt-6">
          <div class="w-16 h-16 rounded-full bg-slate-100 dark:bg-[#151a2e] text-slate-400 flex items-center justify-center mx-auto mb-4">
            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/>
            </svg>
          </div>
          <p class="font-semibold text-slate-700 dark:text-[#f1f3f9]">Şu an aktif kiracınız bulunmamaktadır.</p>
        ...
        <div v-else class="space-y-3 mt-6">
          ...
          <div class="grid grid-cols-2 gap-3 mb-5">
            ...
            <div class="app-card p-4 border-l-4 border-l-blue-500">
              ...
              <p class="text-2xl font-black text-brand-600 dark:text-brand-400">{{ formatCurrency(totalTenantDebt) }}</p>
            </div>
          </div>
          ...
          >
            <div class="flex items-start justify-between gap-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="font-bold text-slate-800 dark:text-[#f1f3f9]">{{ flat.tenantName }}</p>
                  <p class="text-xs text-slate-400 font-medium mt-0.5">Ünite: <span class="text-slate-600 dark:text-[#f1f3f9] font-bold">{{ flat.code }}</span></p>
                </div>
              </div>
            ...
            <!-- Borç kalemleri -->
            <div v-if="getFlatDebts(flat.flatId).length > 0" class="mt-4 space-y-1.5 border-t border-slate-100 dark:border-white/[0.07] pt-3">
              <div
              ...
              >
                ...
                <span class="font-bold text-slate-700 dark:text-[#f1f3f9]">{{ formatCurrency(debt.remainingAmount) }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="emptyFlats.length > 0" class="mt-8">
          ...
          <div class="app-card divide-y divide-slate-50 dark:divide-white/[0.04]">
            <div v-for="flat in emptyFlats" :key="flat.flatId" class="px-4 py-3 flex items-center justify-between">
              <span class="text-sm font-semibold text-slate-600 dark:text-[#f1f3f9]">Ünite {{ flat.code }}</span>
