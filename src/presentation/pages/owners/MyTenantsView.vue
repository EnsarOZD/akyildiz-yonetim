    ...
    <div class="max-w-4xl mx-auto">
      ...
      <template v-else>
        ...
        <div v-if="occupiedFlats.length === 0" class="app-card p-12 text-center mt-6">
          <div class="w-16 h-16 rounded-full bg-slate-100 text-[#8298ab] flex items-center justify-center mx-auto mb-4">
            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/>
            </svg>
          </div>
          <p class="font-semibold text-slate-700">Şu an aktif kiracınız bulunmamaktadır.</p>
          <p class="text-sm text-[#8298ab] mt-1">Mülkleriniz boş veya kiracı bilgisi eklenmemiş.</p>
        </div>
        ...
        <div v-else class="space-y-3 mt-6">
          ...
          <div class="grid grid-cols-2 gap-3 mb-5">
            <div class="app-card p-4 border-l-4 border-l-emerald-500">
              <p class="text-[10px] font-bold text-[#8298ab] uppercase tracking-widest mb-1">Aktif Kiracı</p>
              <p class="text-2xl font-black text-emerald-600">{{ occupiedFlats.length }}</p>
            </div>
            <div class="app-card p-4 border-l-4 border-l-blue-500">
              <p class="text-[10px] font-bold text-[#8298ab] uppercase tracking-widest mb-1">Toplam Kiracı Borcu</p>
              <p class="text-2xl font-black text-brand-600">{{ formatCurrency(totalTenantDebt) }}</p>
            </div>
          </div>
          ...
          >
            <div class="flex items-start justify-between gap-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="font-bold text-slate-800">{{ flat.tenantName }}</p>
                  <p class="text-xs text-[#8298ab] font-medium mt-0.5">Ünite: <span class="text-slate-600 font-bold">{{ flat.code }}</span></p>
                </div>
              </div>
              ...
              <div class="text-right shrink-0">
                ...
                <p class="text-[10px] text-[#8298ab] uppercase font-bold mt-0.5">Bakiye</p>
              </div>
            </div>

            <!-- Borç kalemleri -->
            <div v-if="getFlatDebts(flat.flatId).length > 0" class="mt-4 space-y-1.5 border-t border-slate-100 pt-3">
              <div
              ...
              >
                ...
                <span class="font-bold text-slate-700">{{ formatCurrency(debt.remainingAmount) }}</span>
              </div>
            </div>
          </div>
        </div>
