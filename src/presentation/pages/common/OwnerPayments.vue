    ...
    <div class="max-w-7xl mx-auto">
        ...
        <template #actions>
          <div class="flex items-center gap-3">
            <div class="flex flex-col items-end">
              <span class="text-[10px] font-bold uppercase tracking-wider text-[#8298ab]">Toplam Tahsilat</span>
              <span class="text-sm font-bold text-slate-700">{{ formatCurrency(totalAmount) }}</span>
            </div>
            <div class="w-px h-8 bg-slate-200 mx-1"></div>
            <div class="flex flex-col items-end">
              <span class="text-[10px] font-bold uppercase tracking-wider text-emerald-600">Bu Ay</span>
      ...
      <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
          ...
          <table class="table w-full">
            ...
            <tbody>
              <tr v-for="payment in filteredPayments" :key="payment.id" class="border-b border-gray-200 hover:bg-gray-50">
                <td class="p-4">
                  <div>
                    <p class="font-medium text-gray-800">{{ payment.ownerName }}</p>
                    <p class="text-sm text-gray-500">{{ payment.ownerEmail }}</p>
                  </div>
                </td>
                <td class="p-4 text-gray-800">{{ payment.unit }}</td>
                <td class="p-4">
                  <span 
                    :class="{
                      'bg-purple-100 text-purple-800': payment.type === 'aidat',
                      'bg-brand-100 text-brand-700': payment.type === 'su',
                      'bg-yellow-100 text-yellow-800': payment.type === 'elektrik',
                      'bg-gray-100 text-gray-800': payment.type === 'diger'
                    }"
                ...
                <td class="p-4 text-gray-800">{{ payment.description || '-' }}</td>
                <td class="p-4">
                  <span class="font-semibold text-green-600">{{ formatCurrency(payment.amount) }}</span>
                </td>
                <td class="p-4 text-gray-800">{{ formatDate(payment.date) }}</td>
                <td class="p-4">
        ...
        <div v-if="filteredPayments.length === 0" class="text-center py-12">
          ...
          <p class="text-gray-500">Arama kriterlerinize uygun ödeme kaydı bulunamadı.</p>
        </div>
      </div>
    </div>
  </div>
