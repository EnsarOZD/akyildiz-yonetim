        ...
        <template #actions>
          <div class="flex items-center gap-3">
            <div class="flex flex-col items-end">
              <span class="text-[10px] font-bold uppercase tracking-wider text-[#8298ab]">Toplam Borç</span>
              <span class="text-sm font-bold text-slate-700">{{ formatCurrency(totalDues) }}</span>
            </div>
            <div class="w-px h-8 bg-slate-200 mx-1"></div>
            <div class="flex flex-col items-end">
              <span class="text-[10px] font-bold uppercase tracking-wider text-red-600">Geciken</span>
      ...
      <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
        <div class="overflow-x-auto">
          <table class="table w-full">
            ...
            <tbody>
              <tr v-for="due in filteredDues" :key="due.id" class="border-b border-gray-200 hover:bg-gray-50">
                <td class="p-4">
                  <div>
                    <p class="font-medium text-gray-800">{{ due.ownerName }}</p>
                    <p class="text-sm text-gray-500">{{ due.ownerEmail }}</p>
                  </div>
                </td>
                <td class="p-4 text-gray-800">{{ due.unit }}</td>
                <td class="p-4 text-gray-800">{{ due.period }}</td>
                <td class="p-4">
                  <span class="font-semibold text-gray-800">{{ formatCurrency(due.amount) }}</span>
                </td>
                <td class="p-4 text-gray-800">{{ formatDate(due.dueDate) }}</td>
                <td class="p-4">
                  <span 
                    :class="{
                      'bg-green-100 text-green-800': due.status === 'paid',
                      'bg-yellow-100 text-yellow-800': due.status !== 'paid' && !due.isOverdue,
                      'bg-red-100 text-red-800': due.isOverdue
                    }"
        ...
        <div v-if="filteredDues.length === 0" class="text-center py-12">
          <div class="bg-gray-100 text-gray-500 rounded-full p-3 w-fit mx-auto mb-3">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
            </svg>
          </div>
          <p class="text-gray-500">Arama kriterlerinize uygun aidat kaydı bulunamadı.</p>
        </div>
      </div>
    </div>
  </div>
