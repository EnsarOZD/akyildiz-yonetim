<template>
  <div v-if="isManagerOrViewer || userRole === 'tenant'" class="p-4 sm:p-6 min-h-screen">
    <div class="max-w-7xl mx-auto">
      ...
      <section class="bg-white p-6 rounded-xl shadow-md border border-gray-200 mb-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div class="form-control">
            <label class="label">
              <span class="label-text text-gray-700">Kiracı</span>
            </label>
            <select v-model="filterTenant" class="select select-bordered w-full bg-white border-gray-300 text-gray-700">
          ...
          <div class="form-control">
            <label class="label">
              <span class="label-text text-gray-700">Tip</span>
            </label>
          ...
          <div class="form-control">
            <label class="label">
              <span class="label-text text-gray-700">Dönem</span>
            </label>
            <select v-model="filterPeriod" class="select select-bordered w-full bg-white border-gray-300 text-gray-700">
          ...
          <div class="form-control">
            <label class="label">
              <span class="label-text text-gray-700">Tutar Aralığı</span>
            </label>
            <select v-model="filterAmountRange" class="select select-bordered w-full bg-white border-gray-300 text-gray-700">
      ...
      <section class="bg-white p-6 rounded-xl shadow-md border border-gray-200 mb-8">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-xl font-semibold text-gray-800">Geciken Ödeme Listesi</h2>
          <div class="flex items-center gap-3">
            <div v-if="hasActiveFilters" class="bg-brand-100 text-brand-600 px-3 py-1 rounded-full text-sm font-medium">
              Filtrelenmiş
            </div>
            ...
            <button @click="exportToExcel" class="btn btn-sm btn-ghost border border-gray-300" title="Excel Olarak İndir">
            ...
            <button @click="exportToPDF" class="btn btn-sm btn-ghost border border-gray-300" title="PDF Olarak İndir">
        ...
        <div v-if="filteredOverdueItems.length > 0" class="space-y-3">
          <div 
            ...
            :key="item.id"
            class="flex items-center justify-between p-4 bg-red-50 rounded-lg border border-red-200 hover:bg-red-100 transition-colors duration-200"
          >
            <div class="flex items-center gap-4">
              ...
              <div>
                <p class="font-semibold text-gray-800">{{ item.company }}</p>
                <p class="text-sm text-gray-500">{{ item.unit }} - {{ item.typeLabel }}</p>
                <p class="text-xs text-[#8298ab]">Dönem: {{ item.period || '-' }}</p>
              </div>
            </div>
            <div class="text-right">
              ...
              <p class="font-bold text-red-600 text-lg">{{ formatCurrency(item.unpaidAmount) }}</p>
              <p class="text-sm text-gray-500">Son Ödeme: {{ item.dueDateFormatted }}</p>
              <div class="flex items-center gap-2 text-xs">
                <span class="text-red-500 font-medium">
                  {{ getDaysOverdue(item.dueDate) }} gün gecikmiş
                </span>
                <span v-if="item.paymentStatus.status === 'partial'" class="text-yellow-600">
        ...
        <div v-else class="text-center py-12">
          <div class="bg-green-100 text-green-600 rounded-full p-4 w-fit mx-auto mb-4">
  ...
  <div v-else class="flex items-center justify-center min-h-screen">
    <div class="text-center">
      <div class="bg-red-100 text-red-600 rounded-full p-4 w-fit mx-auto mb-4">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
        </svg>
      </div>
      <p class="text-gray-500 text-lg">Bu sayfayı görüntüleme yetkiniz bulunmamaktadır.</p>
    </div>
  </div>
</template>
