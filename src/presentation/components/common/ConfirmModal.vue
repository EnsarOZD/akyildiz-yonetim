<template>
  <dialog v-if="isOpen" class="modal" open @keydown.esc="$emit('cancel')">
    ...
    >
      ...
      <!-- Content -->
      <div class="flex-grow overflow-y-auto overscroll-contain px-5 py-5 bg-gray-50 dark:bg-gray-900/30 text-gray-700 dark:text-gray-300">
        {{ message }}
      </div>

      <!-- Footer -->
      <div class="flex-shrink-0 flex items-center justify-end gap-3 px-5 py-4 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-b-2xl">
        <button
          type="button"
          class="btn btn-ghost"
          :disabled="loading"
