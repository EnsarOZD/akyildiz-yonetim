        ...
        <div v-if="filteredOverdueItems.length > 0" class="space-y-3">
          ...
          >
            ...
            <div class="text-right">
              <p class="font-bold text-red-600 dark:text-red-400 text-lg">{{ formatCurrency(item.toplamTutar || item.kdvDahil || item.amount) }}</p>
...
const filteredOverdueItems = computed(() => {
  return overdueItems.value.filter(item => {
    // Kiracı filtresi
    ...
    const amount = Number(item.toplamTutar || item.kdvDahil || item.amount || 0)
...
const uniqueTenants = computed(() => {
  const tenants = [...new Set(overdueItems.value.map(item => item.company))]
  return tenants.filter(tenant => tenant && tenant !== 'Bilinmiyor').sort()
})

const uniquePeriods = computed(() => {
  const periods = [...new Set(overdueItems.value.map(item => item.period))]
...
const fetchAidats = async () => {
  ...
  aidats.value = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))
}
...
const buildOverdueList = () => {
  const now = new Date()
  const combined = [...aidats.value, ...readings.value]

  const overdue = combined
    .filter(item => {
      const isOverdue = item.dueDate && new Date(item.dueDate) < now
      const isSharedUtility = item.description?.toLowerCase().includes('ortak') ||
                              item.description?.toLowerCase().includes('mescit')
      const hasValidTenant = !!item.tenantId
      const isZeroWater = item.type === 'water' && Number(item.toplamTutar || 0) === 0

      // Tenant ise sadece kendi kayıtlarını görebilsin
      const isOwnTenantData = userRole.value === 'tenant'
        ? item.tenantId === userId.value
        : true

      return isOverdue && hasValidTenant && !isSharedUtility && !isZeroWater && isOwnTenantData
    })
    .map(item => {
      const tenant = tenants.value.find(t => t.id === item.tenantId)
      return {
        ...item,
        company: tenant?.company || 'Bilinmiyor',
        unit: tenant?.unit || (Array.isArray(tenant?.units) ? tenant.units.join(', ') : '-'),
        typeLabel:
          item.type === 'electricity' ? 'Elektrik' :
          item.type === 'water' ? 'Su' : 'Aidat',
  ...
  overdueItems.value = overdue.sort((a, b) => new Date(b.dueDate) - new Date(a.dueDate))
}
